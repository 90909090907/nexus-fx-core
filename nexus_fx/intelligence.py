...
class EvidenceMemoryEngine:
    """Durable experiment/signal ledger with Supabase persistence.

    When ``SUPABASE_URL`` and ``SUPABASE_KEY`` are available (for example via
    Streamlit Secrets), SIGNAL rows are stored in ``nexus_signals`` and
    EXPERIMENT rows in ``nexus_experiments``. A local SQLite database remains
    available as a safe fallback for local development and for temporary
    Supabase outages; remote failures never prevent the statistical engine from
    starting.

    Experimental memory stays diagnostic-only and does not enter the official
    BUY/SELL probability used by ``decision_mask``.
    """
    ...
    def __init__(self, path: Optional[str] = None) -> None:
        ...
        # Root-level Streamlit secrets are normally exposed as environment
        # variables. This fallback also supports nested/runtime-only secrets.
        ...
        if url and key:
            try:
                ...
                # Lightweight connectivity/schema probe. Do not crash startup
                # if the remote service is temporarily unavailable.
                self.supabase.table("nexus_signals").select("id").limit(1).execute()
                self.supabase.table("nexus_experiments").select("id").limit(1).execute()
                self.remote_enabled = True
            except Exception as exc:
                self.remote_error = f"Supabase unavailable: {exc}"
                self.supabase = None
                self.remote_enabled = False
    ...
    @classmethod
    def _remote_table(cls, kind: str) -> str:
        return cls.REMOTE_TABLES[cls._normalize_kind(kind)]
    ...
    def _remote_filter_key(self, query, payload: Dict[str, object]):
        return (
            query
            .eq("signal_time", payload["signal_time"])
            .eq("source", payload["source"])
            .eq("target", payload["target"])
            .eq("lag", payload["lag"])
            .eq("interval", payload["interval"])
            .eq("direction", payload["direction"])
        )

    def _remote_record(self, payload: Dict[str, object], kind: str) -> bool:
        if not self.remote_enabled or self.supabase is None:
            return False
        table = self._remote_table(kind)
        try:
            lookup = self._remote_filter_key(
                self.supabase.table(table).select("id"), payload
            ).limit(1).execute()
            ...
            self.remote_error = f"Supabase write failed: {exc}"
            return False
    ...
    def record_signal(
        ...
        payload: Dict[str, object] = {
            ...
            "evidence_score": self._safe_float(evidence_score),
            "reference_price": self._safe_float(reference_price),
            "horizon_bars": int(horizon_bars),
            "status": "OPEN",
        }

        # Persist locally as a fallback/cache first.
        with self._connect() as con:
            con.execute(
                """
            ...
            if kind == "SIGNAL":
                con.execute(
                    """
                    UPDATE signals
                    SET kind='SIGNAL', regime=?, probability=?, evidence_score=?,
                        reference_price=?, horizon_bars=?
        ...
        self._remote_record(payload, kind)

    def _settle_local(self, close: pd.DataFrame, interval: str) -> int:
        ...
        with self._connect() as con:
            ...
            for row_id, signal_time, target, direction, ref_price, horizon_bars in rows:
                ...
                con.execute(
                    """
                    UPDATE signals
                    SET status='CLOSED', outcome_time=?, outcome_price=?,
                        signed_return=?, hit=?
                    WHERE id=?
                    """,
                    (
                        self._time_key(idx[outcome_pos]),
                        float(outcome_price), signed_ret, hit, row_id,
                    ),
                )
                settled += 1
        return settled
    ...
    def _settle_remote_table(
        ...
        try:
            ...
            for row in rows:
                ...
                if target not in work.columns:
                    continue
                ts = pd.Timestamp(row.get("signal_time"))
                if idx.tz is not None and ts.tzinfo is None:
                    ts = ts.tz_localize(idx.tz)
                elif idx.tz is None and ts.tzinfo is not None:
                    ts = ts.tz_convert(None)
                ...
                outcome_pos = pos + horizon_bars
                if pos >= len(idx) or outcome_pos >= len(idx):
                    continue
                outcome_price = pd.to_numeric(work[target], errors="coerce").iloc[outcome_pos]
                ref_price = self._safe_float(row.get("reference_price"))
                if ref_price is None or not np.isfinite(outcome_price) or ref_price <= 0:
                    continue
                raw_ret = float(outcome_price / ref_price - 1.0)
                signed_ret = raw_ret if str(row.get("direction", "")).upper() == "BUY" else -raw_ret
                hit = int(signed_ret > 0.0)
                self.supabase.table(table).update({
                    "status": "CLOSED",
                    "outcome_time": self._time_key(idx[outcome_pos]),
            ...
            self.remote_error = f"Supabase settle failed ({table}): {exc}"
        return settled

    def settle(self, close: pd.DataFrame, interval: str) -> int:
        if close is None or close.empty:
            return 0
        local_count = self._settle_local(close, interval)
        remote_count = 0
        if self.remote_enabled:
            remote_count += self._settle_remote_table("nexus_signals", close, interval)
            remote_count += self._settle_remote_table("nexus_experiments", close, interval)
    ...
    def _remote_stats(
        ...
        try:
            ...
            if regime and regime != "UNKNOWN":
                query = query.eq("regime", str(regime))
            response = query.order("id", desc=True).execute()
            rows = getattr(response, "data", None) or []
            trials = len(rows)
            hits = int(sum(int(bool(r.get("hit"))) for r in rows))
            recent = rows[: int(recent_n)]
            recent_trials = len(recent)
            recent_hits = int(sum(int(bool(r.get("hit"))) for r in recent))
            return MemoryStats(
                trials=trials,
                hits=hits,
            ...
            self.remote_error = f"Supabase stats failed ({table}): {exc}"
            return None
    ...
    def _local_stats(
        ...
        with self._connect() as con:
            total = con.execute(
                f"SELECT COUNT(*), COALESCE(SUM(hit),0) FROM signals WHERE {where}",
                params,
            ).fetchone()
            recent = con.execute(
    ...
    def stats(
        ...
        # Current engine asks SIGNAL and EXPERIMENT separately. Prefer Supabase
        # whenever available; otherwise use the local fallback.
        if kind:
            remote = self._remote_stats(
                source, target, lag, interval, regime, recent_n,
                self._normalize_kind(kind),
            )
            if remote is not None:
                return remote
        return self._local_stats(source, target, lag, interval, regime, recent_n, kind)

    def _remote_export_table(self, kind: str) -> List[Dict[str, object]]:
        if not self.remote_enabled or self.supabase is None:
            return []
        table = self._remote_table(kind)
        try:
            response = self.supabase.table(table).select("*").order("id").execute()
            rows = list(getattr(response, "data", None) or [])
            for row in rows:
                row["kind"] = kind
            return rows
        except Exception as exc:
            self.remote_error = f"Supabase export failed ({table}): {exc}"
            return []

    def export_frame(self) -> pd.DataFrame:
        if self.remote_enabled:
            rows = self._remote_export_table("EXPERIMENT") + self._remote_export_table("SIGNAL")
            if rows:
