...
APP_VERSION = "0.2.2.6"
INTELLIGENCE_VERSION = "STAT-INLINE-0.2.2.6"


# -----------------------------------------------------------------------------
# INLINE INTELLIGENCE ENGINE
# Single-file deployment to prevent module-version drift on mobile updates.
# -----------------------------------------------------------------------------
...
class RegimeEngine:
    ...
    v0.2.2.6 adds uncertainty diagnostics to avoid interpreting a concentrated
...
st.set_page_config(page_title="NEXUS FX CORE v0.2.2.6.1", page_icon="◈", layout="wide")
...
if st.button("⚡ Construir estado del mercado", use_container_width=True, type="primary"):
    ...
    try:
        with st.spinner("Sincronizando FX, resolviendo estado latente, régimen y grafo condicional..."):
            ...
            st.session_state["nexus_v0226_result"] = (close,) + result
    except Exception as exc:
        st.exception(exc)

if "nexus_v0226_result" not in st.session_state:
    st.info("Pulsa “Construir estado del mercado”. v0.2.2.6 endurece la evidencia estadística; sigue sin emitir órdenes BUY/SELL.")
    st.stop()

close, latent, regime, graph, divergences, triangles, stress, graph_warning = st.session_state["nexus_v0226_result"]
if graph_warning:
...
with t1:
    st.caption("v0.2.2.6 exige FDR, bootstrap, walk-forward, estabilidad y decay. Una arista sobreviviente sigue siendo una hipótesis predictiva, no causalidad económica demostrada.")
    if graph.empty:
...
with t2:
    st.caption("El posterior de régimen no está externamente calibrado. v0.2.2.6 añade entropía, persistencia y una métrica separada de fiabilidad/autoconsistencia.")
...
with t5:
    st.markdown("**Pipeline de evidencia v0.2.2.6:**")
    st.markdown(
...
st.warning("NEXUS v0.2.2.6 es investigación estadística estructural. No ejecuta operaciones ni recomienda apalancamiento.")
