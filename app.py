...
from nexus_fx.intelligence import RegimeResult, RegimeEngine, CausalGraphEngine, StressResult, NetworkStressEngine

APP_VERSION = "0.2.2.3"
INTELLIGENCE_VERSION = "STAT-MODULE-0.2.2.3"
...
st.set_page_config(page_title="NEXUS FX CORE v0.2.2.3.1", page_icon="◈", layout="wide")
...
if st.button("⚡ Construir estado del mercado", use_container_width=True, type="primary"):
    ...
    try:
        with st.spinner("Sincronizando FX, resolviendo estado latente, régimen y grafo condicional..."):
            ...
            st.session_state["nexus_v0223_result"] = (close,) + result
    except Exception as exc:
        st.exception(exc)

if "nexus_v0223_result" not in st.session_state:
    st.info("Pulsa “Construir estado del mercado”. v0.2.2.3 endurece la evidencia estadística; sigue sin emitir órdenes BUY/SELL.")
    st.stop()

close, latent, regime, graph, divergences, triangles, stress, graph_warning = st.session_state["nexus_v0223_result"]
if graph_warning:
...
with t1:
    st.caption("v0.2.2.3 exige FDR, bootstrap, walk-forward, estabilidad y decay. Una arista sobreviviente sigue siendo una hipótesis predictiva, no causalidad económica demostrada.")
    if graph.empty:
...
with t2:
    st.caption("El posterior de régimen no está externamente calibrado. v0.2.2.3 añade entropía, persistencia y una métrica separada de fiabilidad/autoconsistencia.")
...
with t5:
    st.markdown("**Pipeline de evidencia v0.2.2.3:**")
    st.markdown(
...
st.warning("NEXUS v0.2.2.3 es investigación estadística estructural. No ejecuta operaciones ni recomienda apalancamiento.")
